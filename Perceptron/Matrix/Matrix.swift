...
internal func transpose(_ x: Matrix<Float>) -> Matrix<Float> {
...
// Compute the matrix transpose.
internal func transpose(_ x: Matrix<Double>) -> Matrix<Double> {
...
extension Matrix: Sequence {
    func makeIterator() -> AnyIterator<ArraySlice<T>> {
...
extension Matrix: Equatable {
    static func ==<T>(lhs: Matrix<T>, rhs: Matrix<T>) -> Bool {
...
internal func identity(rows: Int, columns: Int? = nil, offset: Int = 0) -> Matrix<Float> {
...
///       upper diagonal, and a negative value to a lower diagonal.
/// - Returns: A 2-D tensor where all elements are 0, except for the `offset`-th diagonal, whose values are 1.
internal func identity(rows: Int, columns: Int? = nil, offset: Int = 0) -> Matrix<Double> {
    let columns = columns ?? rows
    try! precondition(offset < rows, "offset should be less than or equal to rows.")
...
internal func add(_ x: Matrix<Float>, y: Matrix<Float>) -> Matrix<Float> {
...
internal func add(_ x: Matrix<Double>, y: Matrix<Double>) -> Matrix<Double> {
    try! precondition(x.rows == y.rows && x.columns == y.columns, "Matrix dimensions not compatible with addition")
    var results = y
...
internal func sub(_ x: Matrix<Float>, y: Matrix<Float>) -> Matrix<Float> {
...
internal func sub(_ x: Matrix<Double>, y: Matrix<Double>) -> Matrix<Double> {
    try! precondition(x.rows == y.rows && x.columns == y.columns, "Matrix dimensions not compatible with subtraction")
    var results = y
...
internal func mul(_ alpha: Float, x: Matrix<Float>) -> Matrix<Float> {
    var results = x
    cblas_sscal(Int32(x.data.count), alpha, &(results.data), 1)
    return results
}

internal func mul(_ alpha: Double, x: Matrix<Double>) -> Matrix<Double> {
    var results = x
...
internal func mul(_ x: Matrix<Float>, y: Matrix<Float>) -> Matrix<Float> {
...
// Matrix product of two matrices.
internal func mul(_ x: Matrix<Double>, y: Matrix<Double>) -> Matrix<Double> {
    try! precondition(x.columns == y.rows, "Matrix dimensions not compatible with multiplication")
...
internal func elmul(_ x: Matrix<Double>, y: Matrix<Double>) -> Matrix<Double> {
...
// Multiply matrices element-wise.
internal func elmul(_ x: Matrix<Float>, y: Matrix<Float>) -> Matrix<Float> {
    try! precondition(x.rows == y.rows && x.columns == y.columns, "Matrix must have the same dimensions")
...
// Multiply matrices element-wise.
internal func eldiv(_ x: Matrix<Double>, y: Matrix<Double>) -> Matrix<Double> {
    try! precondition(x.rows == y.rows && x.columns == y.columns, "Matrix must have the same dimensions")
    var result = Matrix<Double>(repeating: 0, rows: x.rows, columns: x.columns)
    result.data = x.data / y.data
    return result
...
// Multiply matrices element-wise.
internal func eldiv(_ x: Matrix<Float>, y: Matrix<Float>) -> Matrix<Float> {
    try! precondition(x.rows == y.rows && x.columns == y.columns, "Matrix must have the same dimensions")
    var result = Matrix<Float>(repeating: 0, rows: x.rows, columns: x.columns)
    result.data = x.data / y.data
    return result
...
internal func div(_ x: Matrix<Double>, y: Matrix<Double>) -> Matrix<Double> {
    let yInv = inv(y)
    try! precondition(x.columns == yInv.rows, "Matrix dimensions not compatible")
    return mul(x, y: yInv)
}

internal func div(_ x: Matrix<Float>, y: Matrix<Float>) -> Matrix<Float> {
    let yInv = inv(y)
    try! precondition(x.columns == yInv.rows, "Matrix dimensions not compatible")
    return mul(x, y: yInv)
}

internal func pow(_ x: Matrix<Double>, _ y: Double) -> Matrix<Double> {
    return Matrix(pow(x.data, y), rows: x.rows, columns: x.columns)
}

internal func pow(_ x: Matrix<Float>, _ y: Float) -> Matrix<Float> {
    return Matrix(pow(x.data, y), rows: x.rows, columns: x.columns)
}

internal func sqrt(_ x: Matrix<Double>) -> Matrix<Double> {
    return Matrix(sqrt(x.data), rows: x.rows, columns: x.columns)
}

internal func sqrt(_ x: Matrix<Float>) -> Matrix<Float> {
    return Matrix(sqrt(x.data), rows: x.rows, columns: x.columns)
}

internal func exp(_ x: Matrix<Double>) -> Matrix<Double> {
    return Matrix(exp(x.data), rows: x.rows, columns: x.columns)
}

internal func exp(_ x: Matrix<Float>) -> Matrix<Float> {
    return Matrix(exp(x.data), rows: x.rows, columns: x.columns)
}
...
internal func apply(_ function: (_ v: Vector<Double>) -> Double, to x: Matrix<Double>,
...
//      - axis: Axis or axes along which a sum is performed. The default is column-wise.
// - Returns: A row or column Matrix of values along the given axis (rows x 1 if axis = .row else 1 x columns).
internal func apply(_ function: (_ v: Vector<Float>) -> Float, to x: Matrix<Float>,
...
internal func apply(_ function: (_ v: Vector<Double>) -> Vector<Double>, to x: Matrix<Double>,
                    along axis: MatrixAxis = .columns) -> Matrix<Double> {
...
//      - axis: Axis or axes along which a sum is performed. The default is column-wise.
// - Returns: A Matrix of the same shape with function applied along the given axis.
internal func apply(_ function: (_ v: Vector<Float>) -> Vector<Float>, to x: Matrix<Float>,
                    along axis: MatrixAxis = .columns) -> Matrix<Float> {
...
internal func apply(_ function: (_ v: Vector<Float>) -> Vector<Float>, to x: Matrix<Float>) -> Matrix<Float> {
...
//      - x: The input matrix.
// - Returns: A Matrix of the same shape with function applied to all elements.
internal func apply(_ function: (_ v: Vector<Double>) -> Vector<Double>, to x: Matrix<Double>) -> Matrix<Double> {
    return Matrix(function(x.data), rows: x.rows, columns: x.columns)
}
...
internal func inv(_ x : Matrix<Float>) -> Matrix<Float> {
...
// - Parameter: matrix Matrix to be inverted.
// - Returns: The (multiplicative) inverse of matrix.
internal func inv(_ x : Matrix<Double>) -> Matrix<Double> {
    try! precondition(x.rows == x.columns, "Matrix must be square")
...
//      print(b, b * (a * b))
internal func pseudoInverse(_ matrix: Matrix<Double>) -> Matrix<Double> {
...
//      print(a, a * (b * a)) // The two sets of matrices should be _almost_ equal
//      print(b, b * (a * b))
internal func pseudoInverse(_ matrix: Matrix<Float>) -> Matrix<Float> {
    let (U, s, V) = singularValueDecomposition(matrix)
    let m = U.rows
    let n = V.columns
    let ma = m < n ? n : m
